...
outblock
    : var_decl_part {displayLlvmcodes(codehd);}
      ...
      statement {RetPush(decltl->rettype);}
    ;
...
subprog_decl_part
	  : /* empty */
	  | subprog_decl_list SEMICOLON {printf("subprog_decl_part end.");}
    ;
...
statement
    ...
    if_statement
    : IF condition {printf("if\n"); BrCondPush(FactorStackPtr->data, reg_cntr, NULL);} THEN {LabelPush(reg_cntr++);} statement {backpatch(reg_cntr);} else_statement {LabelPush(reg_cntr++); printf("if end.\n");}
    ;

    else_statement
    : /* empty */
    | ELSE {BrUncondPush(-1); LabelPush(reg_cntr);} statement {backpatch(reg_cntr);}
    ;
    ...
    for_statement
    : FOR {AllocaPush(makeFactor(LOCAL_VAR, "FOR_COND_VAL", reg_cntr++));}
      IDENT ASSIGN expression {StorePush(FactorStackPtr->data, *lookup($3));}
      TO {LabelPush(reg_cntr++);}
      expression {IcmpPush(SLT, *lookup($3), FactorStackPtr->data, *lookup("FOR_COND_VAL")); BrCondPush(*lookup("FOR_COND_VAL"), reg_cntr, -1);}
      DO {LabelPush(reg_cntr++);}
      statement {BrUncondPush((BrCommandStackPtr->ptr->args.bruncond.arg1)-1); backpatch(reg_cntr); LabelPush(reg_cntr++);}
    ;
    ...
    block_statement
    : SBEGIN statement_list SEND {printf("block_statement end.");}
    ;
    ...
    factor
    : var_name
    | NUMBER {Factor f = makeFactor(CONSTANT, "", $1); FactorPush(&f);}
